...
#     # Correct RMS calculation in linear space
#     rms_bb_spl_linear = np.sqrt(grouped.mean()**2)

#     # Convert back to dB
#     rms_bb_spl = 10 * np.log10(rms_bb_spl_linear)

#     # Compute percentiles in dB domain
#     percentiles = np.array([
#         np.percentile(bb_spl.values, [1, 5, 50, 95, 99])
#         for _ in grouped
#     ])
...
# def segment_bb_spl(ds, segment_duration='30min'):
#     """
#     Segment broadband SPL data and compute RMS and percentiles.
    
#     Parameters
#     ----------
#     ds : xarray.Dataset
#         Dataset containing broadband SPL with time dimension
#     segment_duration : str, optional
#         Time frequency for segmentation 
#         Use pandas/xarray time offset aliases:
#         - '30min' (recommended)
#         - '1h'
#         - '15min'
#         Default is '30min'
...
#     -------
#     tuple
#         (segment_times, rms_bb_spl, percentiles)
#         - segment_times : Center times of each segment
#         - rms_bb_spl : RMS broadband SPL per segment
#         - percentiles : Percentiles (1,5,50,95,99) per segment
#     """
#     # Ensure input is a DataArray of broadband SPL
#     if 'bb_spl' not in ds:
#         raise ValueError("Input dataset must contain 'bb_spl' variable")
...
#     # Compute RMS SPL for each segment (using geometric mean in linear space)
#     rms_bb_spl_linear = grouped.mean()
#     rms_bb_spl = 10 * np.log10(rms_bb_spl_linear)
    
#     # Compute percentiles manually
#     percentiles = []
#     for group in grouped:
#         # Convert to numpy array and compute percentiles
#         group_percentiles = np.percentile(group[1].values, [1, 5, 50, 95, 99])
#         percentiles.append(group_percentiles)
